...
public class King extends AbstractPiece{
  ...
  protected King(V2 position, ChessColor color) {
  ...
  private static List<List<V2>> generatePossibleMoves() {
    ...
    for (final var d : new V2[]{N, S, W, E, NW, SW, NE, SE}) {
      List<V2> inDirection = new ArrayList<>();
      for (int i = 1; i < Math.min(2,ChessBoard.BOARD_SIZE); i++) inDirection.add(d.times(i));
