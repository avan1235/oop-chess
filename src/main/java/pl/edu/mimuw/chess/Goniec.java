...
public class Goniec implements Figura {
  public Kolor kolor;
  public V2 pozycja;


  public Goniec(Kolor kolor, V2 pozycja) {
  ...
  public ArrayList<Ruch> wszystkieRuchy() {
    ...
    V2 poz = new V2(this.pozycja.x-1, this.pozycja.y-1);
    while (poz.naPlanszy()) {
    ...
    poz = new V2(this.pozycja.x+1, this.pozycja.y+1);
    while (poz.naPlanszy()) {
      if (!Plansza.pola.get(poz.x).containsKey(poz.y)) {
        res.add(new Ruch(this, new V2(poz.x, poz.y)));
        poz.y++;
